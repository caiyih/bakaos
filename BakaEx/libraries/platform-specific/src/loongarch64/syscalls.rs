...
impl<TPayload> SyscallPayload<'_, TPayload> {
    ...
    fn arg_i<T: Sized + Copy>(&self, i: usize) -> T {
        ...
        let arg0 = unsafe {
            (self.trap_ctx as *const _ as *const usize).add(4 /* offset of a0 */)
        };

        // Since LoongArch is little-endian, we can safely cast usize to T
        unsafe { arg0.add(i).cast::<T>().read() }
    }

    fn trap_ctx(&self) -> &crate::loongarch64::TaskTrapContext {
        unsafe {
            (self.trap_ctx as *const _ as *const crate::loongarch64::TaskTrapContext)
                .as_ref()
                .unwrap()
        }
    }

    fn trap_ctx_mut(&mut self) -> &mut crate::loongarch64::TaskTrapContext {
        unsafe {
            (self.trap_ctx as *mut _ as *mut crate::loongarch64::TaskTrapContext)
                .as_mut()
                .unwrap()
        }
    }
