...
pub trait DropDelegate<T> = FnOnce(T);

pub struct InvokeOnDrop<T, F: DropDelegate<T>> {
    func: Option<F>,
    val: ManuallyDrop<T>,
}

impl<F: DropDelegate<()>> InvokeOnDrop<(), F> {
    pub fn new(func: F) -> Self {
        Self::transform((), func)
    }
}

impl<T, F: DropDelegate<T>> InvokeOnDrop<T, F> {
...
impl<T, F: DropDelegate<T>> Drop for InvokeOnDrop<T, F> {
    fn drop(&mut self) {
...
impl<T: Copy, F: DropDelegate<T>> InvokeOnDrop<T, F> {
    pub fn as_val(&self) -> T {
        *self.val
    }
}

impl<F: DropDelegate<T>, T> Deref for InvokeOnDrop<T, F> {
    type Target = T;
...
impl<T, F: DropDelegate<T>> DerefMut for InvokeOnDrop<T, F> {
