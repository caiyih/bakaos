...
async_syscall!(sys_futex_async, ctx, {
    ...
    match ctx.arg1::<i32>() /* futex_op */ {
        FUTEX_OP_WAIT => {
            let val = ctx.arg2::<u32>();
            let uaddr = ctx.arg0::<VirtualAddress>();

            // validate uaddr
            ctx.borrow_page_table().guard_ptr(unsafe { uaddr.as_ptr::<u32>() }).mustbe_user().with_read().ok_or(ErrNo::BadAddress)?;

            let p_timeout = ctx.arg3::<*const TimeSpec>();
            let timeout = ctx.borrow_page_table().guard_ptr(p_timeout).mustbe_user().with_read().map(|g| *g);
        ...
        FUTEX_OP_WAKE => return Ok(ctx.pcb.lock().futex_queue.wake(
                ctx.arg0::<VirtualAddress>(), // uaddr
                ctx.arg2::<usize>(), // nval
            ) as isize),
        FUTEX_OP_REQUEUE | FUTEX_OP_CMP_REQUEUE => return Ok(ctx.pcb.lock().futex_queue.requeue(
